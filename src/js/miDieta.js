    ...
    try{
        ...
        conn.query(query, (error,result,fields) => {
            if(error){
                ...
                renderDieta(result,suscripcion)
            }
        });
...
/*
        ...
        */
        ...
        query +=  "UPDATE alimentos_comidas "
        query +=  "SET alimentos_comidas.modificar = 1 "
        query +=  "WHERE alimentos_comidas.alimento = \"" + alimento + "\" AND "
        query +=  "alimentos_comidas.tipo = \"" + comida + "\" "
        query +=  "AND alimentos_comidas.comida = (SELECT idcomidas_dia "
        query +=  "FROM mybalance.comidas_del_dia "
        query += "WHERE mybalance.comidas_del_dia.dia = '" + año + "-" + mes + "-" + dia +"' AND mybalance.comidas_del_dia.dieta IN "
        query += "(SELECT dieta FROM mybalance.dieta "
        query += "WHERE mybalance.dieta.suscripcion = " + suscripcion +")) "
